...
use crate::lens_struct::{Side, Lens};
mod lens_struct;

use crate::ray_vector::{Ray, WFE_Ray, Vector3D, WFE_Stats, dot_product};
mod ray_vector;

use crate::rtm_subs::{calc_slope, calc_dir_sines, calc_aoi_lsa, translate_to_surface, translate_to_flat};
mod rtm_subs;
...
#[no_mangle]
pub extern fn rcalc_wfe(p0: Vector3D, e0: Vector3D, lens: &Lens, refocus: f64) -> f64
{
	...
	let opd = 1000.0f64 * (ymaoi.sin() * ymaoi.sin() / 2.0f64) * (refocus - a * rsq / 2.0f64 - b * rsqsq / 3.0f64) / lens.wl;
	return opd;
}
...
#[no_mangle]
extern fn gen_and_trace_wfe_rays(ptr: *mut WFE_Ray, npts: usize, loopsize: i32, lens: Lens, refocus: f64) -> WFE_Stats
{
	let din:  &mut [WFE_Ray] = unsafe { std::slice::from_raw_parts_mut(ptr, npts) }; 

	gen_wfe_rays(lens.ap, npts, din, loopsize );

	let mut wstats = WFE_Stats{ minopd: 1e20f64, maxopd: -1e20f64, varirms: 0f64};
...
fn calc_wfe_ray(wferay: &mut WFE_Ray, lens: &Lens, refocus: f64)
{
	...
	if rsq < 0.0000001f64
	{
		...
		wferay.rend = Ray{pvector: Vector3D{x: 0f64, y: 0f64, z: lens.ct + lens.bfl + refocus}, edir: CPROPV};  
		wferay.lsa = 0f64;
		wferay.opd = 0f64;
	}
...
#[no_mangle]
pub extern fn trace_full_ray(ray: Ray, lens: &Lens, refocus: f64) -> Ray
{
	// Trace ray from srf 0 to first lens surface. The axial distance here should be zero.
